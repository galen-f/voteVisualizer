...
def _two_votes_per_state(
    df: pd.DataFrame, state_col: str, vote_col: str, senator_col: str | None
):
    """
...
def render_map_senate(
    ...
    base = (
        gdf[[state_col, "geometry"]]
        .drop_duplicates(subset=[state_col])
        .reset_index(drop=True)
    )

    # derive v1, v2 per state from the possibly duplicated rows
    votes2 = _two_votes_per_state(
        gdf[
            [state_col, vote_col]
            + ([senator_col] if senator_col and senator_col in gdf.columns else [])
        ],
        state_col=state_col,
        vote_col=vote_col,
        senator_col=senator_col,
    )
    ...
    states.plot(
        ax=ax, facecolor="#f8f8f8", edgecolor="#444444", linewidth=0.6, zorder=1
    )
    ...
    present_values = [
        v
        for v in ["Yea", "Nay", "Present", "Not Voting"]
